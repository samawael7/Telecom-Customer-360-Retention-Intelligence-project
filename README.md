...
<img width="1339" height="635" alt="image" src="https://github.com/user-attachments/assets/3ee352e8-9005-43d4-b765-0e8d99be19e1" />
...
**ليه ده مهم:** فاتورة قديمة اتحملت وقت ما العميل كان بياناته مختلفة، لازم تفضل مرتبطة بنسخته التاريخية الصح — لو فلترنا على `IS_CURRENT='Y'` بدري قبل الأوان، كنا هنفقد ربط الفواتير القديمة بالكامل من حسابات الإيراد.

### 3) منهج Static Lookup بدل Dynamic Lookup Cache

جُرّب في البداية Dynamic Lookup Cache للـidentity resolution، لكن اتكشف فيه خطر حقيقي: خاصية "Insert Else Update" ممكن تكتب فوق `GOLDEN_CUSTOMER_ID` الصح بقيمة غلط لو القيم الجاية مختلفة عن الكاش. **الحل النهائي:** Static Lookups (قراءة بس)، مع تقنية "ذاكرة بين الصفوف" (variable ports) داخل الـExpression نفسها — أضمن وأبسط في الـdebugging.
...
|---|---|---|
| عالي | عالي | **CRITICAL** |
| عالي | منخفض |**HIGH_VALUE_STABLE** |
| منخفض | عالي |**MONITOR** |
| منخفض | منخفض | **LOW_PRIORITY** |
| عالي | منخفض HIGH_VALUE_AT_RISK |
...
- **NULL matching في Static Lookup Cache:** رقم تليفون فاضي كان بيتربط غلط بأول عميل NULL في الكاش (عميل واحد ابتلع 210 فاتورة بالغلط) — اتحل بشرط صريح `NOT ISNULL()` قبل تصديق أي نتيجة Lookup.
- **Column truncation صامت:** عمود `CHANNEL_NAME` كان ضيق جدًا فقصّ قيمة "Call Center" بصمت، وده فوّت 1595 صف من المطابقة — اتحل بتوسيع العمود ومراجعة كل أعمدة الأبعاد بعدها.
- **Case-sensitivity mismatch:** قيم `PAYMENT_METHOD` بين المصدر والـDimension كانت بحالة أحرف مختلفة (`CASH` مقابل `Cash`) — اتحل بتوحيد الحالة قبل أي مقارنة.
- **Full-history reprocessing على target غير فاضي:** إعادة تشغيل مابينج SCD2 على مصدر كامل (شامل النسخ التاريخية) بعد ما الـtarget بقى فيه بيانات، كانت بتسبب تعارض في القيود — اتحل بمنطق يفرّق بين بيانات "اتحمّلت قبل كده" و"تحديث حقيقي جديد".

---

## ✅ التحقق من الجودة (Verified)

- Idempotency test: تشغيل الـWorkflow مرتين من غير بيانات جديدة → صفر تغيير في كل الجداول
- End-to-end test: إضافة عميل جديد كامل (CRM + فاتورة + تذكرة) → كل جدول زاد بالظبط بواحد
- Orphan record check: صفر سجلات "يتيمة" في كل الـFacts وRETENTION_PRIORITY_LIST
- Data reconciliation: كل صفوف المصدر = صفوف ناجحة + صفوف مرفوضة موثّقة، بالظبط
